/**
 * Generated by orval v8.23.0 🍺
 * Do not edit manually.
 * Api
 * Adaptive Learning Support App API
 * OpenAPI spec version: 0.1.0
 */
import * as zod from 'zod';


/**
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  "status": zod.string()
})


/**
 * @summary Get current user
 */
export const GetMeResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Login with email and password
 */
export const LoginBody = zod.object({
  "email": zod.string(),
  "password": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']).optional()
})

export const LoginResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Register a new user
 */
export const RegisterBody = zod.object({
  "email": zod.string(),
  "password": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "gradeLevel": zod.number().optional(),
  "age": zod.number().optional()
})

export const RegisterResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Exchange a Firebase Authentication ID token for an app session
 */
export const FirebaseSessionBody = zod.object({
  "idToken": zod.string().describe('The Firebase Authentication ID token from the frontend\'s signed-in user'),
  "displayName": zod.string().optional(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']).optional(),
  "gradeLevel": zod.number().optional(),
  "age": zod.number().optional()
})

export const FirebaseSessionResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Logout
 */
export const LogoutResponse = zod.object({
  "success": zod.boolean(),
  "message": zod.string().optional()
})


/**
 * @summary List all users (admin/teacher)
 */
export const listUsersQueryPageDefault = 1;
export const listUsersQueryLimitDefault = 20;

export const ListUsersQueryParams = zod.object({
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']).optional(),
  "page": zod.coerce.number().default(listUsersQueryPageDefault),
  "limit": zod.coerce.number().default(listUsersQueryLimitDefault)
})

export const ListUsersResponse = zod.object({
  "users": zod.array(zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
})),
  "total": zod.number(),
  "page": zod.number(),
  "limit": zod.number()
})


/**
 * @summary Get a user by ID
 */
export const GetUserParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetUserResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Update user profile
 */
export const UpdateUserParams = zod.object({
  "userId": zod.coerce.number()
})

export const UpdateUserBody = zod.object({
  "displayName": zod.string().optional(),
  "avatarUrl": zod.string().optional(),
  "gradeLevel": zod.number().optional(),
  "age": zod.number().optional()
})

export const UpdateUserResponse = zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Delete a user
 */
export const DeleteUserParams = zod.object({
  "userId": zod.coerce.number()
})

export const DeleteUserResponse = zod.object({
  "success": zod.boolean(),
  "message": zod.string().optional()
})


/**
 * @summary Get learning progress for a user
 */
export const GetUserProgressParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetUserProgressResponse = zod.object({
  "userId": zod.number(),
  "totalLessonsCompleted": zod.number(),
  "totalTimeMinutes": zod.number(),
  "overallAccuracy": zod.number(),
  "currentStreak": zod.number(),
  "longestStreak": zod.number(),
  "level": zod.number(),
  "xp": zod.number(),
  "stars": zod.number(),
  "coins": zod.number()
})


/**
 * @summary Get completed lessons list
 */
export const ListCompletedLessonsParams = zod.object({
  "userId": zod.coerce.number()
})

export const ListCompletedLessonsQueryParams = zod.object({
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']).optional()
})

export const ListCompletedLessonsResponseItem = zod.object({
  "id": zod.number(),
  "userId": zod.number(),
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "lessonId": zod.string(),
  "lessonTitle": zod.string(),
  "score": zod.number(),
  "accuracy": zod.number(),
  "timeSpentMinutes": zod.number(),
  "completedAt": zod.string()
})
export const ListCompletedLessonsResponse = zod.array(ListCompletedLessonsResponseItem)


/**
 * @summary Record a lesson completion
 */
export const RecordProgressBody = zod.object({
  "userId": zod.number(),
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "lessonId": zod.string(),
  "lessonTitle": zod.string(),
  "score": zod.number(),
  "accuracy": zod.number(),
  "timeSpentMinutes": zod.number()
})

export const RecordProgressResponse = zod.object({
  "id": zod.number(),
  "userId": zod.number(),
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "lessonId": zod.string(),
  "lessonTitle": zod.string(),
  "score": zod.number(),
  "accuracy": zod.number(),
  "timeSpentMinutes": zod.number(),
  "completedAt": zod.string()
})


/**
 * @summary List all possible achievements
 */
export const ListAchievementsResponseItem = zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "description": zod.string(),
  "icon": zod.string(),
  "category": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing', 'games', 'streaks', 'general']),
  "xpReward": zod.number(),
  "coinsReward": zod.number()
})
export const ListAchievementsResponse = zod.array(ListAchievementsResponseItem)


/**
 * @summary Get achievements for a user
 */
export const GetUserAchievementsParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetUserAchievementsResponseItem = zod.object({
  "achievement": zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "description": zod.string(),
  "icon": zod.string(),
  "category": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing', 'games', 'streaks', 'general']),
  "xpReward": zod.number(),
  "coinsReward": zod.number()
}),
  "unlockedAt": zod.string()
})
export const GetUserAchievementsResponse = zod.array(GetUserAchievementsResponseItem)


/**
 * @summary Get rewards for a user
 */
export const GetUserRewardsParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetUserRewardsResponse = zod.object({
  "userId": zod.number(),
  "stars": zod.number(),
  "coins": zod.number(),
  "xp": zod.number(),
  "level": zod.number(),
  "xpToNextLevel": zod.number(),
  "unlockedAvatars": zod.array(zod.string()),
  "activeAvatar": zod.string()
})


/**
 * @summary Award stars, coins, XP to a user
 */
export const AwardRewardsBody = zod.object({
  "userId": zod.number(),
  "stars": zod.number().optional(),
  "coins": zod.number().optional(),
  "xp": zod.number().optional(),
  "reason": zod.string().optional()
})

export const AwardRewardsResponse = zod.object({
  "userId": zod.number(),
  "stars": zod.number(),
  "coins": zod.number(),
  "xp": zod.number(),
  "level": zod.number(),
  "xpToNextLevel": zod.number(),
  "unlockedAvatars": zod.array(zod.string()),
  "activeAvatar": zod.string()
})


/**
 * @summary Get game leaderboard
 */
export const getLeaderboardQueryLimitDefault = 10;

export const GetLeaderboardQueryParams = zod.object({
  "game": zod.enum(['math-sprint', 'memory-match', 'word-builder', 'balloon-pop', 'treasure-hunt', 'shape-match', 'letter-match', 'sentence-builder', 'number-puzzle', 'picture-puzzle']),
  "limit": zod.coerce.number().default(getLeaderboardQueryLimitDefault)
})

export const GetLeaderboardResponseItem = zod.object({
  "rank": zod.number(),
  "userId": zod.number(),
  "displayName": zod.string(),
  "avatarUrl": zod.string().nullish(),
  "score": zod.number(),
  "playedAt": zod.string()
})
export const GetLeaderboardResponse = zod.array(GetLeaderboardResponseItem)


/**
 * @summary Submit a game score
 */
export const SubmitScoreBody = zod.object({
  "userId": zod.number(),
  "game": zod.enum(['math-sprint', 'memory-match', 'word-builder', 'balloon-pop', 'treasure-hunt', 'shape-match', 'letter-match', 'sentence-builder', 'number-puzzle', 'picture-puzzle']),
  "score": zod.number(),
  "coinsEarned": zod.number(),
  "starsEarned": zod.number()
})

export const SubmitScoreResponse = zod.object({
  "id": zod.number(),
  "userId": zod.number(),
  "displayName": zod.string(),
  "game": zod.string(),
  "score": zod.number(),
  "coinsEarned": zod.number(),
  "starsEarned": zod.number(),
  "playedAt": zod.string()
})


/**
 * @summary Get scores for a user
 */
export const GetUserScoresParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetUserScoresResponseItem = zod.object({
  "id": zod.number(),
  "userId": zod.number(),
  "displayName": zod.string(),
  "game": zod.string(),
  "score": zod.number(),
  "coinsEarned": zod.number(),
  "starsEarned": zod.number(),
  "playedAt": zod.string()
})
export const GetUserScoresResponse = zod.array(GetUserScoresResponseItem)


/**
 * @summary Get today's daily challenge
 */
export const GetTodayChallengeResponse = zod.object({
  "date": zod.string(),
  "mathQuestions": zod.array(zod.object({
  "id": zod.string(),
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "question": zod.string(),
  "options": zod.array(zod.string()),
  "correctAnswer": zod.string(),
  "hint": zod.string()
})),
  "englishQuestions": zod.array(zod.object({
  "id": zod.string(),
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "question": zod.string(),
  "options": zod.array(zod.string()),
  "correctAnswer": zod.string(),
  "hint": zod.string()
})),
  "phonicsQuestions": zod.array(zod.object({
  "id": zod.string(),
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "question": zod.string(),
  "options": zod.array(zod.string()),
  "correctAnswer": zod.string(),
  "hint": zod.string()
}))
})


/**
 * @summary Submit daily challenge answers
 */
export const SubmitDailyChallengeBody = zod.object({
  "userId": zod.number(),
  "date": zod.string(),
  "answers": zod.array(zod.object({
  "questionId": zod.string(),
  "answer": zod.string()
}))
})

export const SubmitDailyChallengeResponse = zod.object({
  "totalCorrect": zod.number(),
  "totalQuestions": zod.number(),
  "earnedBadge": zod.boolean(),
  "coinsEarned": zod.number(),
  "starsEarned": zod.number(),
  "xpEarned": zod.number()
})


/**
 * @summary Check if user completed today's challenge
 */
export const GetDailyChallengeStatusParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetDailyChallengeStatusResponse = zod.object({
  "userId": zod.number(),
  "date": zod.string(),
  "completed": zod.boolean(),
  "score": zod.number().nullish(),
  "earnedBadge": zod.boolean()
})


/**
 * @summary Student dashboard summary
 */
export const GetStudentDashboardParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetStudentDashboardResponse = zod.object({
  "user": zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
}),
  "rewards": zod.object({
  "userId": zod.number(),
  "stars": zod.number(),
  "coins": zod.number(),
  "xp": zod.number(),
  "level": zod.number(),
  "xpToNextLevel": zod.number(),
  "unlockedAvatars": zod.array(zod.string()),
  "activeAvatar": zod.string()
}),
  "recentProgress": zod.array(zod.object({
  "id": zod.number(),
  "userId": zod.number(),
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "lessonId": zod.string(),
  "lessonTitle": zod.string(),
  "score": zod.number(),
  "accuracy": zod.number(),
  "timeSpentMinutes": zod.number(),
  "completedAt": zod.string()
})),
  "subjectBreakdown": zod.array(zod.object({
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "lessonsCompleted": zod.number(),
  "totalLessons": zod.number(),
  "accuracy": zod.number(),
  "timeMinutes": zod.number()
})),
  "recentAchievements": zod.array(zod.object({
  "achievement": zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "description": zod.string(),
  "icon": zod.string(),
  "category": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing', 'games', 'streaks', 'general']),
  "xpReward": zod.number(),
  "coinsReward": zod.number()
}),
  "unlockedAt": zod.string()
})),
  "dailyChallengeCompleted": zod.boolean(),
  "weeklyStreak": zod.number()
})


/**
 * @summary Parent dashboard — progress of linked children
 */
export const GetParentDashboardParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetParentDashboardResponse = zod.object({
  "children": zod.array(zod.object({
  "user": zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
}),
  "progress": zod.object({
  "userId": zod.number(),
  "totalLessonsCompleted": zod.number(),
  "totalTimeMinutes": zod.number(),
  "overallAccuracy": zod.number(),
  "currentStreak": zod.number(),
  "longestStreak": zod.number(),
  "level": zod.number(),
  "xp": zod.number(),
  "stars": zod.number(),
  "coins": zod.number()
}),
  "recentLessons": zod.array(zod.object({
  "id": zod.number(),
  "userId": zod.number(),
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "lessonId": zod.string(),
  "lessonTitle": zod.string(),
  "score": zod.number(),
  "accuracy": zod.number(),
  "timeSpentMinutes": zod.number(),
  "completedAt": zod.string()
})),
  "subjectBreakdown": zod.array(zod.object({
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "lessonsCompleted": zod.number(),
  "totalLessons": zod.number(),
  "accuracy": zod.number(),
  "timeMinutes": zod.number()
}))
})),
  "weeklyActivity": zod.array(zod.object({
  "date": zod.string(),
  "lessonsCompleted": zod.number(),
  "timeMinutes": zod.number(),
  "accuracy": zod.number()
}))
})


/**
 * @summary Teacher dashboard — all students summary
 */
export const GetTeacherDashboardParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetTeacherDashboardResponse = zod.object({
  "totalStudents": zod.number(),
  "avgAccuracy": zod.number(),
  "activeToday": zod.number(),
  "topPerformers": zod.array(zod.object({
  "user": zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
}),
  "accuracy": zod.number(),
  "lessonsCompleted": zod.number(),
  "lastActive": zod.string()
})),
  "studentsNeedingHelp": zod.array(zod.object({
  "user": zod.object({
  "id": zod.number(),
  "email": zod.string(),
  "displayName": zod.string(),
  "role": zod.enum(['student', 'parent', 'teacher', 'admin']),
  "avatarUrl": zod.string().nullish(),
  "gradeLevel": zod.number().nullish(),
  "age": zod.number().nullish(),
  "parentId": zod.number().nullish(),
  "createdAt": zod.string()
}),
  "accuracy": zod.number(),
  "lessonsCompleted": zod.number(),
  "lastActive": zod.string()
})),
  "subjectStats": zod.array(zod.object({
  "subject": zod.string(),
  "avgAccuracy": zod.number(),
  "totalLessonsCompleted": zod.number()
}))
})


/**
 * @summary Admin dashboard — platform-wide analytics
 */
export const GetAdminDashboardResponse = zod.object({
  "totalStudents": zod.number(),
  "totalTeachers": zod.number(),
  "totalParents": zod.number(),
  "activeToday": zod.number(),
  "totalLessonsCompleted": zod.number(),
  "avgAccuracy": zod.number(),
  "topGames": zod.array(zod.object({
  "game": zod.string(),
  "timesPlayed": zod.number(),
  "avgScore": zod.number()
})),
  "registrationsThisWeek": zod.number()
})


/**
 * @summary Weekly learning activity chart data
 */
export const GetWeeklyProgressParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetWeeklyProgressResponseItem = zod.object({
  "date": zod.string(),
  "lessonsCompleted": zod.number(),
  "timeMinutes": zod.number(),
  "accuracy": zod.number()
})
export const GetWeeklyProgressResponse = zod.array(GetWeeklyProgressResponseItem)


/**
 * @summary Per-subject accuracy and completion breakdown
 */
export const GetSubjectProgressParams = zod.object({
  "userId": zod.coerce.number()
})

export const GetSubjectProgressResponseItem = zod.object({
  "subject": zod.enum(['math', 'english', 'phonics', 'science', 'geography', 'pshe', 'typing']),
  "lessonsCompleted": zod.number(),
  "totalLessons": zod.number(),
  "accuracy": zod.number(),
  "timeMinutes": zod.number()
})
export const GetSubjectProgressResponse = zod.array(GetSubjectProgressResponseItem)


/**
 * @summary Assign a typing curriculum module to a student (parent/teacher)
 */
export const AssignTypingModuleBody = zod.object({
  "studentId": zod.number(),
  "levelId": zod.string(),
  "moduleId": zod.string(),
  "note": zod.string().optional()
})

export const AssignTypingModuleResponse = zod.object({
  "id": zod.number(),
  "studentId": zod.number(),
  "assignedBy": zod.number(),
  "levelId": zod.string(),
  "moduleId": zod.string(),
  "note": zod.string().nullish(),
  "createdAt": zod.string(),
  "completed": zod.boolean()
})


/**
 * @summary List a student's assigned typing modules, with completion status
 */
export const ListTypingAssignmentsParams = zod.object({
  "studentId": zod.coerce.number()
})

export const ListTypingAssignmentsResponseItem = zod.object({
  "id": zod.number(),
  "studentId": zod.number(),
  "assignedBy": zod.number(),
  "levelId": zod.string(),
  "moduleId": zod.string(),
  "note": zod.string().nullish(),
  "createdAt": zod.string(),
  "completed": zod.boolean()
})
export const ListTypingAssignmentsResponse = zod.array(ListTypingAssignmentsResponseItem)


/**
 * @summary Remove a typing module assignment
 */
export const DeleteTypingAssignmentParams = zod.object({
  "id": zod.coerce.number()
})

export const DeleteTypingAssignmentResponse = zod.object({
  "success": zod.boolean(),
  "message": zod.string().optional()
})


