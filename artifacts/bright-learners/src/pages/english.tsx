...
export default function English() {
  ...
  const { data: completedLessons } = useListCompletedLessons(
    user?.id || 0,
    { subject: 'english' },
    { query: { queryKey: getListCompletedLessonsQueryKey(user?.id || 0, { subject: 'english' }), enabled: !!user?.id } },
  );
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          ...
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <div className="bg-white dark:bg-card rounded-2xl sm:rounded-3xl p-3 sm:p-4 shadow-lg shrink-0">
              <BookText className="w-10 h-10 sm:w-16 sm:h-16 text-blue-600" />
            </div>
            <div className="min-w-0">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-white drop-shadow-lg truncate">English</h1>
              <p className="text-sm sm:text-lg md:text-xl font-bold text-white/95 drop-shadow truncate">Reading & Writing</p>
            </div>
          </div>
        </div>
      </div>
